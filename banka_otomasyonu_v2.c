...
void HesapOzeti();
void HesapKapat();
void RaporAl(int menuTipi); // menuTipi = ana menü den mi geldi müþteri menüden mi 0 ANA MENÜ 1 MÜÞTERÝ MENÜ
...
void RaporAl(int menuTipi) // menuTipi = ana menü den mi geldi müþteri menüden mi 0 ANA MENÜ 1 MÜÞTERÝ MENÜ
{
	...
	printf("\n\t<!> Bankanýn Geliri: %.2f TL",bnk.gelir);
	printf("\n\t<!> Bankanýn Gideri: %.2f TL",bnk.gider);
	printf("\n\t<!> Banka Hazinesi: %.2f TL",bnk.hazine);
	printf("\n\t<!> Bir önceki menüye dönmek için bir tuþa basýnýz..");
	*secim = getch();
	if(!menuTipi){
			AnaMenu();
			return;
...
void HesapKapat()
{
	...
	if(!bnk.aktif_musteri.hesapsayisi)
		printf("\n\t<!> Gösterilecek hiç bir hesabýnýz yok...\n");
	...
	for(i=0;i<bnk.aktif_musteri.hesapsayisi;i++){
		...
		printf("\n\t<!> Kapatmak istediðiniz hesap ID'sini giriniz (Hesabýnýzý kapatmak için içinde para olmamalýdýr) (-1 Menüye Dön): ");
		scanf("%s",&secim);
	...
	{
		printf("\n\t<!> Hesabýnýzdaki para 0 deðil! Mevcut miktar: %.2f",bnk.aktif_musteri.hesap[hesapID].bakiye);
		Sleep(3000);
		HesapKapat();
		return;
	}
	printf("\n\t<!> %d No'lu hesabýnýz kapatýlacak, devam etmek istiyorsanýz bir tuþa basýnýz.. (0= Ýptal Et ve Menüye Dön)");
	*secim = getch();
...
void HesaptanParaCek()
{
	...
	*secim = getch();
	...
	{
		printf("\n\t<!> Günlük para çekme limitinizi geçiyorsunuz.");
...
void BankaVerileriniCek()
{
	...
	if(fp == NULL){
			//Daha once hic bu dosya açýlmamýþ
			BankaVerileriniGuncelle();
		}
...
void MusteriVerileriniCek()
{
	...
	if(bireyselF == NULL){
			//Daha once hic bu dosya açýlmamýþ o yüzden oluþtur dosyayý
			bireyselF = fopen("bireysel_musteriler.txt","w");
			fclose(bireyselF);
			MusteriVerileriniCek();
		}
	if(ticariF == NULL){
			//Daha once hic bu dosya açýllmamis o yüzden oluþtur dosyayý
			ticariF = fopen("ticari_musteriler.txt","w");
			fclose(ticariF);
			MusteriVerileriniCek();
		}

	//Bireysel Müþterileri Programa ï¿½ek
	for(i=0;i<bnk.bireyselMS;i++){
